...
CI, pre-commit 훅, 기타 무인 워크플로우용 옵션입니다. `--yes` 는 **어떤 프롬프트도
띄우지 않습니다.** 생성된 PR을 바로 제출하며, 인터랙티브로 물어볼 모든 선택을 자동으로 해결합니다:

- **템플릿** — `-t/--template` 또는 `--auto` 를 주지 않으면 브랜치명/커밋에서 자동 감지합니다.
- **base 브랜치** — `-b/--base` 를 주지 않으면 기본 base 브랜치를 사용합니다.
- **액션 / 자동 머지** — `[y]/[n]/[f]/[e]` 와 자동 머지 프롬프트를 건너뜁니다 (자동 머지는 비활성 유지 — 활성화하려면 `--auto-merge` 와 함께 사용).
